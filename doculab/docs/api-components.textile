...
| Components Line-Items<br/>@/subscriptions/<subscription_id>/components@ | List component line-items belonging to a Subscription | - | - | - |
| Component Line-Item<br/>@/subscriptions/<subscription_id>/components/<y>@ | Read a component line-item belonging to a Subscription | - | _Deprecated_ Update a component line-item belonging to a Subscription | - |
...
h3. Update Component for a Subscription - Deprecated
...
This method of updating a component is deprecated and should not be used.  See the documentation for the "Allocations API":/api-allocations instead.

This method will not create an allocation change event or webhook, and does not have the option to prorate and charge for a partial billing period.  If it is used, it will silently change the component quantity on the subscription, and the new quantity will be used at the next renewal.
