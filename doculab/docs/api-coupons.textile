...
When creating a coupon, you must specify the @product_ids@ that you want this coupon valid for.

Previously you needed to specify a @product_family_id@. This is *deprecated* but still supported. If you do not supply @product_ids@ but you supply a @product_family_id@, the coupon will be associated to all the products in that product family. If no @product_ids@ or @product_family_id@ is passed, the first product family available is used.
...
* @product_ids@ An array of products that this coupon is valid for.
* *Deprecated* @product_family_id@. The ID of the product family to which the coupon belongs.  The coupon can be applied *only* to products in this product family.
...
<pre><code>
      <?xml version="1.0" encoding="UTF-8"?>
        <coupon>
          ...
          <product_ids type="array"><product_id type="integer">1</product_id></product_ids>
        </coupon>
</code></pre>
...
URL: @https://<subdomain>.chargify.com/coupons/validate.<format>?code=<coupon_code>&product_id=<product_id>@
Method: @GET@
Required Parameters:  coupon_id or coupon_code and product_id
...
*Deprecated*: Instead of a @product_id@ you can pass in a @product_family_id@. If you do not specify either a @product_id@ or a @product_family_id@ the coupon code will be validated against the *first* product family.
...
<pre><code>https://<subdomain>.chargify.com/coupons/find.<format>?code=<coupon_code></code></pre>

Or

*Deprecated*: <pre><code>https://<subdomain>.chargify.com/coupons/find.<format>?code=<coupon_code>&product_family_id=1</code></pre>
