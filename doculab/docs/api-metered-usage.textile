...
URL: @https://<subdomain>.chargify.com/components.xml@
Method: @GET@  
Required Parameters:
Optional Parameters: @product_family_id@
...
Feature: Chargify Metered Component Usage XML API
  ...
  Scenario: List metered component usages for a subscription
    ...
    And the site has a metered component
...
h3(#api-list-comp-xml). XML List Components

<pre><code>
Scenario: List metered components
  Given I have 1 product
  And the site has a metered component
  When I send a GET request to https://[@subdomain].chargify.com/components.xml
  Then the response status should be "200 OK"
  And the response should be a "components" array with 1 "component" element
</code></pre>

h3(#api-list-comp-json). JSON List Components

<pre><code>
Scenario: List metered components
  Given I have 1 product
  And the site has a metered component
  When I send a GET request to https://[@subdomain].chargify.com/components.json
  Then the response status should be "200 OK"
