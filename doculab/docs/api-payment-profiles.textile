...
Creating a new payment profile for a customer via the API will not make that payment profile current for any of the customer's subscriptions. Currently the only way to make "switch" payment profiles on a Subscription is via the Admin UI.
