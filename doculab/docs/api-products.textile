...
* *Deprecated*: @product_family@ Nested attributes pertaining to the product family to which this product belongs
...
* @coupon_ids@ An array of coupon ids that can be used for this product
* @component_ids@ An array of component ids that can be used for this product
...
* @handle@ The product API handle
* @description@ The product description
* *Deprecated*: @product_family_id@ The ID of the product family to which the product belongs.
* @accounting_code@ The accounting code (has no bearing in Chargify, may be used within your app)
* @interval_unit@ A string representing the interval unit for this product, either @month@ or @day@
* @interval@ The numerical interval.  i.e. an @interval@ of '30' coupled with an @interval_unit@ of 'day' would mean this product would renew every 30 days
* @initial_charge_in_cents@ The up front charge you have specified.
...
h3. *Deprecated*: List products for a Product Family
    ...
    """
      ...
      <product>
        ...
        <coupon_ids type="array">
            <coupon_id type="integer">1</coupon_id>
        </coupon_ids>
        <component_ids type="array">
            <component_id type="integer">1</component_id>
        </component_ids>
      </product>
      """
...
h3(#api-usage-xml-product-family-products-list). *Deprecated* XML List Products for Product Family Example
...
h3(#api-usage-json-product-family-products-list). *Deprecated* JSON List Products for Product Family Example
...
Feature: Chargify API XML Product create
  ...
  Scenario: Create a product successfully
    Given I have this xml product data
      ...
      ""
    When I send a POST request with the xml data to https://[@subdomain].chargify.com/products.xml
...
Feature: Chargify API JSON Product create
  ...
  Scenario: Create a product successfully
    Given I have this json product data
      ...
      ""
    When I send a POST request with the json data to https://[@subdomain].chargify.com/products.json
    Then the response status should be "201 Created"
    And the response should be the json:
    ""
     {
