...
When creating a subscription, you must specify a product and a customer.  Payment details may be required, depending on the options for the Product being subscribed (see "Product Options":/product-options).
...
* @payment_profile_attributes@ (this may also be referred to as @credit_card_attributes@ or @bank_account_attributes@ or @paypal_account_attributes@)
** @first_name@ (Optional) First name on account.  If omitted, the @first_name@ from customer attributes will be used.
** @last_name@ (Optional) Last name on account.  If omitted, the @last_name@ from customer attributes will be used.
...
** @paypal_email@ (Required when creating a subscription with PayPal) Email address for display on the Payment Details tab.
** @payment_method_nonce@ (Required when creating a subscription with PayPal, but ignored in favor of the @vault_token@, which must be supplied. To create a subscription using only the nonce, see the "V2 Signups API":/api-signups .)
...
** @vault_token@ The "token" provided by your vault storage for an already stored payment profile.pa
