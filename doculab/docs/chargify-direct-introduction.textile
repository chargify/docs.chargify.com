...
# Chargify redirects to a URI you define, adding in signed parameters in the query string information about the result of the original request
...
*Unless you have registered a default redirection URI for the endpoint you are accessing, one of the parameters in your secure data should be for "@redirect_uri@":#redirection-uri.*
...
For example, the following form would attempt to create a Signup resource at Chargify and then redirect to @https://www.example.com@ (with the "result parameters":#results in the query string):
