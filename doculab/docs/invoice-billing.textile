...
You may email any Invoice to the Customer at any time by using the button near bottom of the Invoice view.  There are also links for previewing the text and HTML-email versions of the invoice.  Chargify sends all Invoice emails as multi-part with both text/plain and text/html parts so that the client may choose which to render.
