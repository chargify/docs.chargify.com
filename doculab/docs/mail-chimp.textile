...
Now that the MailChimp integration is enabled, you have the option to disable it in order to make changes, or to completely start over.

!/images/doculab/mailchimp-disable-start-over-5.png!

h2. How it Works

Once you click the "Start Exporting" button, Chargify will sync the first name, last name, subscription state, and current product of all of your customers to the MailChimp list you selected.

Going forward, we will sync changes when:
...
h3. What happens if a customer's email address is changed in Chargify?
