...
For us to connect to your Salesforce account, it must allow access to the Salesforce API. This comes standard on Salesforce Enterprise plans. API access may also be added to other Salesforce plans for an extra fee.  Alternately, you may use a Salesforce Developer account (not a free trial) for testing.
...
No, only new subscriptions that are created _after_ you enable the integration will be sent to Salesforce.
