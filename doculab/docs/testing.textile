...
Chargify sites that are in *test mode* will work with both the "Chargify Test Gateway":http://docs.chargify.com/gateway-configuration *and with the test gateways provided by the gateways* like Authorize.Net, PaymentExpress, Beanstream, etc.  Note that all of the major gateway providers will give you a free test or "developer's" account so that you can test your integration.
