...
If Chargify experiences multiple failures when trying to get a successful acknowledgement from a configured endpoint, this begins a process of pausing or disabling the inoperative endpoints.
...
An endpoint's failure count can be reset back to zero if either:
* You change the endpoint's URL in the site's Settings tab, or
