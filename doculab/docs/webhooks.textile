...
h2. Webhook Acknowledgement and Automatic Retries
...
| @billing_date_change@ | Any change to the billing date that is initiated explicitly by altering the billing date via the admin UI or the API.  This will not be triggered upon a normal renewal and period advancement, or a migration. | @site@, @subscription@<sup>1</sup> |
...
The resource objects sent as payload typically contain the same information as the corresponding API resource.  Site payload objects contain the site's @id@ and @subdomain@. Examples of payloads for each type of event are given at the end of this page.
...
* @successful@ A boolean flag describing whether the webhook was accepted by the webhook endpoint
...
* @last_error_at@ Timestamp indicating when the last non-acceptance occurred
